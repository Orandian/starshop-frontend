...
export const DeliveryInfoSection = ({
  ...
  const shippingAddressData = userDetail?.shipping_address?.[0];
  const shippingName = shippingAddressData?.name;
  const shippingPhoneNumber = shippingAddressData?.phoneNumber;
  ...
  const shippingAddress = useMemo(
    ...
    [shippingName, shippingPhoneNumber, fullAddress],
  );
  ...
  const form = useForm<FormValues>({
    ...
    defaultValues: {
      ...
      address2: userDetail?.shipping_address?.[0]?.address2 || "",
      postalCode: userDetail?.shipping_address?.[0]?.postalCode || "",
      prefecture: userDetail?.shipping_address?.[0]?.prefecture || "",
    },
  });
  ...
  // Initial load
  useEffect(() => {
    if (userDetail?.shipping_address) {
      fetchAddress(
        userDetail.shipping_address?.[0]?.postalCode || "",
        userDetail.shipping_address?.[0]?.address2 || "",
      );
    }
  }, [userDetail?.shipping_address, fetchAddress]);
  ...
  const handleCancel = () => {
    ...
    if (userDetail?.shipping_address?.[0]?.postalCode) {
      fetchAddress(
        userDetail.shipping_address[0].postalCode,
        userDetail.shipping_address[0].address2 || "",
      );
    ...
    <>
      <div>
        <div className="flex items-center gap-2">
          <h3 className="py-4 text-primary mr-2">配達先情報</h3>
          {!isUserLoading && (
            <Button
              variant="outline"
              size="icon"
