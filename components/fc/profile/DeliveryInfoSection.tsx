...
import { deliveryInfoFormSchema, DeliveryInfoFormValues } from "@/lib/schema";
...
export const DeliveryInfoSection = ({
   ...
   * @returns void
   */
  const onSubmit = (values: DeliveryInfoFormValues) => {
    ...
    userAddressUpdate(updateData, {
      onSuccess: () => {
        toast.success("配達先情報が正常に更新されました");
  ...
  return (
    <>
      <InformationSection
        ...
        title="配達先情報"
        items={[
          { label: "配送先名", value: userShippingData?.name || "-" },
