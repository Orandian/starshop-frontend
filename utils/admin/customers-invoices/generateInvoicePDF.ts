...
export const generateInvoicePDF = async (
  ...
  try {
          ...
          <div>
            <div>
              <p style="font-size: 0.875rem; line-height: 1.25rem; margin: 0 0 0.5rem 0;">
                請求番号: <span>${invoiceData?.idNumber || "-"}</span>
              </p>
              <p style="font-size: 0.875rem; line-height: 1.25rem; margin: 0 0 1rem 0;">
                請求日: <span>${invoiceData?.createdAt && convertToJapaneseDate2(invoiceData.createdAt)}</span>
              </p>
            </div>
            <div style="margin-top: 1rem;">
              <h3 style="
                ...
                margin: 0 0 1rem 0;
              ">
                ${userDetail?.company_info?.username || "-"}
              </h3>
              <p style="
                font-size: 0.875rem;
                line-height: 1.25rem;
            ...
            <div style="border-bottom: 4px double black; padding-bottom: 0.5rem;">
              <h3 style="font-weight: 700; margin: 1rem 0 0 0;">
                請求金額 <span style="margin-left: 5rem;">${formatCurrency(invoiceData?.totalAmount || 0)}</span>
              </h3>
            </div>
          </div>
      ...
      const footerContent = page === totalPages - 1 ? `
        <tfoot>
          ...
          <tr>
            ...
            <td style="padding: 0.5rem; text-align: right;">${formatCurrency(totals.totalTax)}</td>
          </tr>
          <tr>
    ...
    const fileName = `請求書-${invoiceData.idNumber}.pdf`;
    pdf.save(fileName);
    return true;
  } catch (error) {
